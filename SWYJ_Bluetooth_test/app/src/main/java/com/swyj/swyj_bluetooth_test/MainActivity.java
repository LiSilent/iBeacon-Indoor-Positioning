        ...
        if (enable) {
            ...
            mHandler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    //得到定位结果
                    /*coord.add(runPositioning(scan_iBeacons));

                    for (int i = 0;i < coord.size();i++) {
                        System.out.println("x:" + coord.get(i)[0] + ",y:" + coord.get(i)[1]);
                    }*/
                    double[] new_coord = runPositioning(scan_iBeacons);
                    System.out.println("x:" + new_coord[0] + ",y:" + new_coord[1]);

                    scan_iBeacons.clear();//清空上一次定位残留下的设备

                    //清空上一次残留RSSI，将设备discover设置为false
                    for (int i = 0;i < installed_iBeacons.size();i++) {
                        installed_iBeacons.get(i).setRSSI_Value(new ArrayList<>());
                        installed_iBeacons.get(i).setDiscover(false);
                    }
